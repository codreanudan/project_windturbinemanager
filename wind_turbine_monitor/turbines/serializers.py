...
class WindTurbineSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        capacity_kw = validated_data.get('capacity_kw')

        try:
            weather_service = WeatherAPIService("c67cac5327c741a7abb82828252505")
            weather_data = weather_service.get_current_weather(latitude, longitude)
            wind_speed = weather_data.get("wind_kph")
        except Exception:
            wind_speed = None
        ...
        if wind_speed:
            ...
            power_w = self.calculate_power_wind(wind_speed, rotor_radius)
            power_kw = power_w / 1000
            instant_output = min(power_kw, capacity_kw)
        else:
            instant_output = 0.0

        validated_data['instant_output_kw'] = instant_output
        return super().create(validated_data)

    def update(self, instance, validated_data):
        # la update, putem recalcula în funcție de noile coordonate sau capacity_kw
        instance.latitude = validated_data.get('latitude', instance.latitude)
        instance.longitude = validated_data.get('longitude', instance.longitude)
        instance.capacity_kw = validated_data.get('capacity_kw', instance.capacity_kw)

        try:
            weather_service = WeatherAPIService("c67cac5327c741a7abb82828252505")
            weather_data = weather_service.get_current_weather(instance.latitude, instance.longitude)
            wind_speed = weather_data.get("wind_kph")
        except Exception:
            wind_speed = None
        ...
        if wind_speed:
            ...
            power_w = self.calculate_power_wind(wind_speed, rotor_radius)
            power_kw = power_w / 1000
            instance.instant_output_kw = min(power_kw, instance.capacity_kw)
        else:
            instance.instant_output_kw = 0.0

        # Setăm și restul câmpurilor normale
        instance.name = validated_data.get('name', instance.name)
        instance.installation_date = validated_data.get('installation_date', instance.installation_date)
        instance.last_maintenance_date = validated_data.get('last_maintenance_date', instance.last_maintenance_date)
        instance.maintenance_done = validated_data.get('maintenance_done', instance.maintenance_done)
